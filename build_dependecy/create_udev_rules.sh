...
if [ -f "/etc/udev/rules.d/98-omo-r1.rules" ]; then
    ...
    echo 'KERNEL=="ttyUSB1", ATTRS{idVendor}=="10c4", ATTRS{idProduct}=="ea60", MODE:="0666", GROUP:="dialout", SYMLINK+="ttyMotor"' > /etc/udev/rules.d/98-omo-r1.rules
...
if [ -f "/etc/udev/rules.d/ydlidar.rules" ]; then
    ...
    echo 'KERNEL=="ttyUSB0", ATTRS{idVendor}=="10c4", ATTRS{idProduct}=="ea60", MODE:="0666", GROUP:="dialout",  SYMLINK+="ttyLiDAR"' >/etc/udev/rules.d/97-ydlidar.rules
